<?php
/**
 * Installaion hooks for MP Stacks + SermonGrid
 *
 ...
 * @since 1.0
 * @global $wpdb
 * @global $mp_stacks_sermongrid_options
 * @global $wp_version
 * @return void
 */
function mp_stacks_sermongrid_install() {
	...
	wp_remote_post( 'http://tracking.mintplugins.com', array(
		...
		'body' => array( 
			'mp_track_event' => true, 
			'event_product_title' => 'MP Stacks + SermonGrid', 
	...
	$mp_stacks_sermongrid_activation = true;

}
register_activation_hook( MP_STACKS_SERMONGRID_PLUGIN_FILE, 'mp_stacks_sermongrid_install' );
 ...
 * @since 1.0
 * @global $wpdb
 * @global $mp_stacks_sermongrid_activation
 * @return void
 */
function mp_stacks_sermongrid_activation_shutdown(){
	
	global $mp_stacks_sermongrid_activation;
	
	if ( $mp_stacks_sermongrid_activation ){
		//Flush the rewrite rules
		flush_rewrite_rules();
	}
	
}
add_action( 'shutdown', 'mp_stacks_sermongrid_activation_shutdown' );